...
def noise_instance(noise_type: DigitalNoiseType, zero: bool = False) -> DigitalNoiseInstance:
    if noise_type in NOISE_single_prob:
        errors = 0.1 if not zero else 0.0
    elif noise_type == DigitalNoiseType.PAULI_CHANNEL:
        errors = (0.4, 0.5, 0.1) if not zero else (0.0,) * 3
    else:
        errors = (0.2, 0.8) if not zero else (0.0,) * 2
...
def test_noisy_primitive(gate_fn: Callable, noise_type: DigitalNoiseType, zero: bool) -> None:
